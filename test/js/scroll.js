$(document).ready(function(){
    let sec02_obj_top//sec02오브젝트의 위에서부터 떨어진값
    let sec03_obj_top
    let sec04_obj_top
    let sec05_obj_top
    ...
    let offset = 200 //콘텐츠가 하단에서부터 offset값만큼 올라오면 애니메이션
    ...
    function scroll_chk(){
        ...
        if((sec03_obj_top - scrolling - window_h) < -offset){
            ...
            $('.sec03 .photo').removeClass(class_name)
        }
    ...
    $(window).resize(function(){
        scroll_chk() //브라우저의 사이즈가 변경되었을때마다 1번실행
    })
    $(window).scroll(function(){
        scroll_chk() //브라우저를 스크롤 할 때마다 1번 실행
    })
})//document.ready